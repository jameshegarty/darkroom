...
function bilinearDemosaic(in1)
  ...
  return im dem(x,y)
    ...
    r_tr = ( in1(x,y+1) + in1(x,y-1) )/[uint8](2) -- red top right
    r_bl = ( in1(x-1,y) + in1(x+1,y) )/[uint8](2) -- red bottom left
    
    r_tl_a = in1(x+1,y+1); r_tl_b = in1(x-1,y-1)
    r_tl_c = in1(x-1,y+1); r_tl_d = in1(x+1,y-1)
    r_tl = (r_tl_a+r_tl_b+r_tl_c+r_tl_d)/[uint8](4) -- red top left
    ...
    -- build green
    g_tl_a = in1(x+1,y); g_tl_b = in1(x-1,y)
    g_tl_c = in1(x,y+1); g_tl_d = in1(x,y-1)
    g_tl = ( g_tl_a + g_tl_b + g_tl_c + g_tl_d )/[uint8](4) -- green top left

    g_br_a = in1(x+1,y); g_br_b = in1(x-1,y)
    g_br_c = in1(x,y+1); g_br_d = in1(x,y-1)
    g_br = (g_br_a+g_br_b+g_br_c+g_br_d)/[uint8](4) -- green bottom right
    ...
    -- build blue
    b_bl = ( in1(x,y+1) + in1(x,y-1) )/[uint8](2) -- blue bottom left

    b_tr = ( in1(x-1,y) + in1(x+1,y) )/[uint8](2) -- blue top right
    
    b_br_a = in1(x+1,y+1); b_br_b = in1(x-1,y-1)
    b_br_c = in1(x-1,y+1); b_br_d = in1(x+1,y-1)
    b_br = (b_br_a+b_br_b+b_br_c+b_br_d)/[uint8](4) -- blue bottom right
...
function doccm(in1)
  return im(x,y) { darkroom.dot(in1(x,y),[ccm[1]]),
