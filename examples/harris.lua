...
function convertToIllum( T , in1 )
  local im convertToIll(x,y) [uint16](clip( orion.sum( in1(x,y)[0]*T[0] , in1(x,y)[1]*T[1] , in1(x,y)[2]*T[2] ) , 0 , 32767 )) end 
  return convertToIll
end
...
function convolve_1_5_( T,R , in1 )
  ...
  local im convolve(x,y)  [int32](clip( (orion.sum( in1(x,y-2)*T[0],in1(x,y-1)*T[1],in1(x,y)*T[2],in1(x,y+1)*T[3],in1(x,y+2)*T[4] ) >> 8 ) , -32767 , 32767 )) end
  local im conv_1_5_(x,y) orion.crop([int16](convolve(x,y))) end 
  return conv_1_5_
end
function convolve_5_1_( T,R , in1 )
  -- TODO -> Convert T to tap 
  local im upCast(x,y) [int32](in1(x,y)) end 
  local im convolve(x,y) [int32](clip( (orion.sum( in1(x-2,y)*T[0],in1(x-1,y)*T[1],in1(x,y)*T[2],in1(x+1,y)*T[3],in1(x+2,y)*T[4] ) >> 8 ) , -32767 , 32767 )) end
  local im conv_5_1_(x,y) orion.crop([int16](convolve(x,y))) end 
  return conv_5_1_
end
...
function sobel( R, in0 )
  ...
  local im in1y(x,y) [int32](clip( orion.sum( -in1(x-1,y-1),-(in1(x,y-1)<<1),-in1(x+1,y-1),in1(x-1,y+1),in1(x,y+1)<<1,in1(x+1,y+1) ) , -32767 , 32767 )) end -- Sobel Y
  local im in1x(x,y)  [int32](clip( orion.sum( -in1(x-1,y-1),-(in1(x-1,y)<<1),-in1(x-1,y+1),in1(x+1,y-1),in1(x+1,y)<<1,in1(x+1,y+1) ) , -32767 , 32767 )) end -- Sobel X
...
function response( T , K , in1 )
  ...
  local im Resp(x,y) orion.crop([int16]( clip( Resp1(x,y) , -32767 , 32767 ) )) end
  return Resp
end
...
function  nms(  T , P1 , in1 )
  ...
  local im NMS(x,y) orion.crop([uint8]( if Pk(x,y) then 255 else 0 end)) end
  return NMS 
end
...
function harris(in1)
   ...
   local P  = darkroomSimple.tap(int, "Peak")
   darkroomSimple.setTap( P , 128 )
   local K  = darkroomSimple.tap(orion.type.int(32), "K")
   darkroomSimple.setTap( K , 1 )
   local I  = { 54 , 183 , 18 } 
